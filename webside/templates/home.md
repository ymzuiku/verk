...
Verk 的核心特点是把每一个 html 都作为组件进行引用、实例化，并且提供了 props、slot、renderHook 等组件常用的模式。
...
这会导致每次输入都有5000个元素被派发更新，虽然[Verk]的状态管理性能很高, [Verk]没有 diff-DOM 开销，直接派发元素对象更新；但是在这种极端条件下我们在输入框输入明显能感觉到页面开始卡顿了。
...
[Verk]约定，组件要传递 slot， 需要使用 `<template>` 包裹元素，其目的是为了 slot 的组件嵌套。
